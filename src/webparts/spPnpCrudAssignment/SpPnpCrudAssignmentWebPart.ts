...
import styles from './SpPnpCrudAssignmentWebPart.module.scss';
import * as strings from 'SpPnpCrudAssignmentWebPartStrings';

export interface ISpPnpCrudAssignmentWebPartProps {
  description: string;
}

export default class SpPnpCrudAssignmentWebPart extends BaseClientSideWebPart<ISpPnpCrudAssignmentWebPartProps> {
  ...
  public readAllItems(): void {
    ...
    html += `<th>ID</th><th>Title</th><th>Name</th><th>MobileNumber</th><th>Gender</th><th>Email</th><th>Address</th>`;

  pnp.sp.web.lists.getByTitle("StudentRegistration").items.get().then((items: any[]) => {
  ...
  private deleteListItem(): void {
    ...
    pnp.sp.web.lists.getByTitle("StudentRegistration").items.getById(id).delete();
  ...
  private updateListItem(): void {
    ...
    pnp.sp.web.lists.getByTitle("StudentRegistration").items.getById(id).update({
  ...
  private readListItem(): void {
    ...
    pnp.sp.web.lists.getByTitle("StudentRegistration").items.getById(id).get().then((item: any) => {

      document.getElementById("txtTitle")["value"] = item["Title"];

      document.getElementById("txtName")["value"] = item["Name"];

      document.getElementById("txtMobileNo")["value"] = item["MobileNumber"];
  ...
  private addListItem() : void {
    ...
    pnp.sp.web.lists.getByTitle("StudentRegistration").items.add({
